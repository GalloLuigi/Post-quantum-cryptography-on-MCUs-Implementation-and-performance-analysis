...
#include "Falcon/falcon.h"
    ...
    uint32_t CLOCK1, CLOCK2, CLOCK_test_nist_KAT;
	...
	const char *hexpkey, const char *const *kat, uint8_t *tmp, size_t tlen)
{
	...
	if (!Zf(compute_public)(h2, f, g, logn, (uint8_t *)(h2 + n))) {
		...
		Serial.println("compute_public failed");				
		exit(EXIT_FAILURE);
	}
	...
	if (!Zf(complete_private)(G2, f, g, F, logn, (uint8_t *)(G2 + n))) {
		...
		Serial.println("Complete_private failed");				
		exit(EXIT_FAILURE);
	}
	...
	if (len1 != 1 + (((n * 14) + 7) >> 3)) {
		...
		Serial.println("Unexpected public key length");				
		exit(EXIT_FAILURE);
	}
	if (tmp[0] != logn) {
		//fprintf(stderr, "unexpected first pkey byte: %u\n", tmp[0]);
		Serial.println("unexpected first pkey byte");				
		exit(EXIT_FAILURE);
	}
	len1 --;
	if (tlen < 2 * len1) {
	...
	if (len2 != len1) {
		...
		Serial.println("Wrong encoded public key length");				
		exit(EXIT_FAILURE);
	}
...
//		fprintf(stderr, "Insufficient buffer size\n");
	...
	for (u = 0; kat[u] != NULL; u += 3) {
		...
		Serial.println("Invalid sig KAT");				
			exit(EXIT_FAILURE);
		}
		len1 --;
		...
		if (!Zf(verify_raw)(c0, s2, h2, logn, (uint8_t *)(c0 + n))) {
			...
			Serial.println("KAT signature failed");				
			exit(EXIT_FAILURE);
		}
	...
	unsigned logn, uint8_t *tmp)
{
	...
	for (i = 0; i < 100; i ++) {
		...
		for (u = 0; u < n; u ++) {
			if (hm2[u] != hm[u]) {
				Serial.println("hash_to_point() mismatch");
				exit(EXIT_FAILURE);
			}
		}
		Zf(sign_dyn)(sig, &rng, f, g, F, G, hm, logn, tt);
		if (!Zf(verify_raw)(hm, sig, h, logn, tt)) {
			Serial.println("self signature (dyn) not verified");
			exit(EXIT_FAILURE);
		}
...
test_keygen_inner(unsigned logn, uint8_t *tmp)
{
	...
	for (i = 0; i < 12; i ++) {
		...
		Zf(to_ntt_monty)(h, logn);
		if (!Zf(verify_raw)(hm, sig, h, logn, tt)) {
			Serial.println("self signature not verified");
			exit(EXIT_FAILURE);
		}
		if (!Zf(verify_recover)(h2, hm, s1, sig, logn, tt)) {
			Serial.println("self signature recovery failed");
			exit(EXIT_FAILURE);
		}
		Zf(to_ntt_monty)(h2, logn);
