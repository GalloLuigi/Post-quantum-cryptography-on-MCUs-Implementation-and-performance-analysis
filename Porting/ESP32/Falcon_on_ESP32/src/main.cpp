...
uint32_t CLOCK_kp, CLOCK_enc, CLOCK_dec;
    unsigned long startTime, endTime, elapsedTime_test_nist_KAT=0,elapsedTime_encode=0,elapsedTime_sig=0,elapsedTime_vsig=0;
...
hextobin(uint8_t *buf, size_t max_len, const char *src)
{
	...
	for (;;) {
		...
		if (c == 0) {
			if (z) {
        Serial.println(F("Lone hex nibble"));
				exit(EXIT_FAILURE);
			}
			return u;
		}
      ...
      Serial.println(F("Not an hex digit"));
			exit(EXIT_FAILURE);
		}
		if (z) {
			if (u >= max_len) {
        Serial.println(F("Hex string too long for buffer"));
				exit(EXIT_FAILURE);
			}
	...
	uint8_t *tmp, size_t tlen)
{
	...
	out = ref + olen;
	if (tlen < ilen + 2 * olen) {
    Serial.println(F("Temporary buffer too short"));
		exit(EXIT_FAILURE);
	}
...
test_SHAKE256(void)
{
  ...
  Serial.print(F("Test SHAKE256:"));
	fflush(stdout);
	tlen = 1000;
	//aggiunta cast
	tmp = (uint8_t *)xmalloc(tlen);
	...
	for (i = 0; i < 10; i ++) {
		...
		for (bits = 4; bits <= 12; bits ++) {
			...
			if (len1 != (((n * bits) + 7) >> 3)) {
				...
				Serial.println(F("ERR trim_i16 encode(0)"));
				exit(EXIT_FAILURE);
			}
			...
			if (len1 != (((n * bits) + 7) >> 3)) {
				...
				Serial.println(F("ERR trim_i16 encode"));
				exit(EXIT_FAILURE);
			}
			...
			if (len1 == 0) {
				//fprintf(stderr,
				//	"ERR comp encode: %zu\n", len1);
				Serial.println(F("ERR comp encode"));
				exit(EXIT_FAILURE);
			}
		...
		for (bits = 4; bits <= 8; bits ++) {
			...
			if (len1 != (((n * bits) + 7) >> 3)) {
				...
				Serial.println(F("ERR trim_i8 encode(0)"));
				exit(EXIT_FAILURE);
			}
...
test_codec(void)
{
	...
	Serial.print(F("Test encode/decode:"));				
	fflush(stdout);
	tlen = 8192;
	tmp = (uint8_t *) xmalloc(tlen);
	...
	const char *hexpkey, const char *const *kat, uint8_t *tmp, size_t tlen)
{
	...
	if (!Zf(complete_private)(G2, f, g, F, logn, (uint8_t *)(G2 + n))) {
		...
		Serial.println(F("Complete_private failed"));				
		exit(EXIT_FAILURE);
	}
	...
	if (len1 != 1 + (((n * 14) + 7) >> 3)) {
		...
		Serial.println(F("Unexpected public key length"));				
		exit(EXIT_FAILURE);
	}
	if (tmp[0] != logn) {
		//fprintf(stderr, "unexpected first pkey byte: %u\n", tmp[0]);
		Serial.println(F("unexpected first pkey byte"));				
		exit(EXIT_FAILURE);
	}
	len1 --;
	if (tlen < 2 * len1) {
	...
	if (len2 != len1) {
		...
		Serial.println(F("Wrong encoded public key length"));				
		exit(EXIT_FAILURE);
	}
...
//		fprintf(stderr, "Insufficient buffer size\n");
	...
	for (u = 0; kat[u] != NULL; u += 3) {
		...
		Serial.println(F("Invalid sig KAT"));				
			exit(EXIT_FAILURE);
		}
		len1 --;
		...
		if (!Zf(verify_raw)(c0, s2, h2, logn, (uint8_t *)(c0 + n))) {
			...
			Serial.println(F("KAT signature failed"));				
			exit(EXIT_FAILURE);
		}
...
test_RNG(void)
{
	...
	for (u = 0; u < (sizeof KAT_RNG_1) / sizeof(KAT_RNG_1[0]); u ++) {
		if (KAT_RNG_1[u] != prng_get_u64(&p)) {
			...
			Serial.println(F("ERR KAT_RNG_1"));
			exit(EXIT_FAILURE);
		}
	}
	for (u = 0; u < (sizeof KAT_RNG_2) / sizeof(KAT_RNG_2[0]); u ++) {
		if (KAT_RNG_2[u] != prng_get_u8(&p)) {
			...
			Serial.println(F("ERR KAT_RNG_2"));
			exit(EXIT_FAILURE);
		}
	}
...
test_FP_block(void)
{
	...
	Serial.println(F("Test floating-point (block): "));				
	fflush(stdout);
	...
	for (ctr = 0; ctr < num; ctr ++) {
		...
		for (u = 0; u < n; u ++) {
			if (fpr_rint(f[u]) != fpr_rint(g[u])) {
				Serial.println(F("FFT/iFFT error"));				
				exit(EXIT_FAILURE);
			}
		}
		...
		for (u = 0; u < n; u ++) {
			if (fpr_rint(f[u]) != fpr_rint(h[u])) {
				Serial.println(F("FFT mul error"));				
				exit(EXIT_FAILURE);
			}
		}
		...
		for (u = 0; u < n; u ++) {
			if (fpr_rint(g[u]) != fpr_rint(h[u])) {
				Serial.println(F("split/merge error"));				
				exit(EXIT_FAILURE);
			}
		}
...
test_poly(void)
{
	...
	Serial.print(F("Test polynomials: "));
...
check_gaussian0_sampler(const u72 *x, int r)
{
	...
	if (z != r) {
		...
		Serial.println(F("wrong half-Gaussian output"));
...
test_gaussian0_sampler(void)
{
	...
	Serial.print(F("Test half-Gaussian sampler: "));
	...
	if (u72_add_small(&t, 1) != 1) {
		Serial.println(F(" wrong distribution sum."));
		exit(EXIT_FAILURE);
	}
...
#define NUM_SAMPLES   100000
	...
	for (ctr = 0; ctr < NUM_SAMPLES; ctr ++) {
		...
		if (z < -MAX_DEV || z > +MAX_DEV) {
			Serial.println(F("out-of-range sampled value"));
			exit(EXIT_FAILURE);
		}
		zz[z + MAX_DEV] ++;
	}
	...
	if (df == 13) {
		if (!fpr_lt(chi, chi_dt_13)) {
			Serial.println(F("chi-square test fail"));
			exit(EXIT_FAILURE);
		}
	} else if (df == 14) {
		...
		Serial.println(F("unexpected number of classes:"));
		exit(EXIT_FAILURE);
	}
...
test_sampler(void)
{
	...
	Serial.print(F("Test sampler:"));

	fflush(stdout);

	inner_shake256_init(&rng);
	//inner_shake256_inject(&rng, (const void *)"test sampler", 12);
	inner_shake256_inject(&rng, (const uint8_t *)"test sampler", 12);
	inner_shake256_flip(&rng);
	...
	unsigned logn, uint8_t *tmp)
{
	...
	for (i = 0; i < 100; i ++) {
		...
		for (u = 0; u < n; u ++) {
			if (hm2[u] != hm[u]) {
				Serial.println(F("hash_to_point() mismatch"));
				exit(EXIT_FAILURE);
			}
		}
		Zf(sign_dyn)(sig, &rng, f, g, F, G, hm, logn, tt);
		if (!Zf(verify_raw)(hm, sig, h, logn, tt)) {
			Serial.println(F("self signature (dyn) not verified"));
			exit(EXIT_FAILURE);
		}
...
test_keygen_inner(unsigned logn, uint8_t *tmp)
{
	...
	for (i = 0; i < 12; i ++) {
		...
		Zf(to_ntt_monty)(h, logn);
		if (!Zf(verify_raw)(hm, sig, h, logn, tt)) {
			Serial.println(F("self signature not verified"));
			exit(EXIT_FAILURE);
		}
		if (!Zf(verify_recover)(h2, hm, s1, sig, logn, tt)) {
			Serial.println(F("self signature recovery failed"));
			exit(EXIT_FAILURE);
		}
		Zf(to_ntt_monty)(h2, logn);
...
test_external_API_inner(unsigned logn, shake256_context *rng)
{
	...
	tmpkg = (uint8_t *)xmalloc(tmpkg_len);
	tmpmp = (uint8_t *)xmalloc(tmpmp_len);
	tmpsd = (uint8_t *)xmalloc(tmpsd_len);
	tmpst = (uint8_t *)xmalloc(tmpst_len);
	tmpvv = (uint8_t *)xmalloc(tmpvv_len);
	tmpek = (uint8_t *)xmalloc(tmpek_len);
	...
	for (i = 0; i < 12; i ++) {
		...
		if (r != 0) {
			Serial.println(F("makepub failed"));
			exit(EXIT_FAILURE);
		}
		...
		if (r != (int)logn) {
			Serial.println(F("get_logn failed"));	
			exit(EXIT_FAILURE);
		}
		...
		if (r != 0) {
			Serial.println(F("sign_dyn failed"));	
			exit(EXIT_FAILURE);
		}
		...
		if (logn >= 5) {
			...
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify err"));	
				exit(EXIT_FAILURE);
			}
		}
		...
		r = falcon_sign_dyn(rng, sigpad, &sigpad_len, FALCON_SIG_PADDED,
			...
			"data1", 5, tmpsd, tmpsd_len);
		if (r != 0) {
			Serial.println(F("sign_dyn(padded) failed"));	
			exit(EXIT_FAILURE);
		}
		if (sigpad_len != FALCON_SIG_PADDED_SIZE(logn)) {
			Serial.println(F("sign_dyn(padded): wrong length"));	
			exit(EXIT_FAILURE);
		}
		r = falcon_verify(sigpad, sigpad_len, FALCON_SIG_PADDED,
			pubkey, pubkey_len, "data1", 5, tmpvv, tmpvv_len);
		if (r != 0) {
			Serial.println(F("verify(padded) failed"));	
			exit(EXIT_FAILURE);
		}
		if (logn >= 5) {
			r = falcon_verify(sigpad, sigpad_len, FALCON_SIG_PADDED,
				...
				tmpvv, tmpvv_len);
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify(padded) err"));	
				exit(EXIT_FAILURE);
			}
		}
		...
		r = falcon_sign_dyn(rng, sigct, &sigct_len, FALCON_SIG_CT,
			...
			"data1", 5, tmpsd, tmpsd_len);
		if (r != 0) {
			Serial.println(F("sign_dyn(ct) failed"));	
			exit(EXIT_FAILURE);
		}
		if (sigct_len != FALCON_SIG_CT_SIZE(logn)) {
			Serial.println(F("sign_dyn(ct): wrong length"));	
			exit(EXIT_FAILURE);
		}
		r = falcon_verify(sigct, sigct_len, FALCON_SIG_CT,
			pubkey, pubkey_len, "data1", 5, tmpvv, tmpvv_len);
		if (r != 0) {
			Serial.println(F("verify(ct) failed"));	
			exit(EXIT_FAILURE);
		}
		if (logn >= 5) {
			r = falcon_verify(sigct, sigct_len, FALCON_SIG_CT,
				...
				tmpvv, tmpvv_len);
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify(ct) err"));	
				exit(EXIT_FAILURE);
			}
		}
		...
		if (r != 0) {
			Serial.println(F("expand_privkey failed"));	
			exit(EXIT_FAILURE);
		}
		...
		if (r != 0) {
			Serial.println(F("sign_tree failed"));	
			exit(EXIT_FAILURE);
		}
		r = falcon_verify(sig, sig_len, FALCON_SIG_COMPRESSED,
			pubkey, pubkey_len, "data1", 5, tmpvv, tmpvv_len);
		if (r != 0) {
			Serial.println(F("verify2 failed"));	
			exit(EXIT_FAILURE);
		}
		if (logn >= 5) {
			r = falcon_verify(sig, sig_len, FALCON_SIG_COMPRESSED,
				...
				tmpvv, tmpvv_len);
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify2 err"));	
				exit(EXIT_FAILURE);
			}
		}
		...
		r = falcon_sign_tree(rng, sigpad, &sigpad_len,
			...
			"data1", 5, tmpst, tmpst_len);
		if (r != 0) {
			Serial.println(F("sign_tree(padded) failed"));	
			exit(EXIT_FAILURE);
		}
		r = falcon_verify(sigpad, sigpad_len, FALCON_SIG_PADDED,
			pubkey, pubkey_len, "data1", 5, tmpvv, tmpvv_len);
		...
		if (logn >= 5) {
			r = falcon_verify(sigpad, sigpad_len, FALCON_SIG_PADDED,
				...
				tmpvv, tmpvv_len);
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify2(padded) err"));	
				exit(EXIT_FAILURE);
			}
		}
		...
		r = falcon_sign_tree(rng, sigct, &sigct_len, FALCON_SIG_CT,
			...
			"data1", 5, tmpst, tmpst_len);
		if (r != 0) {
			Serial.println(F("sign_tree(ct) failed"));	
			exit(EXIT_FAILURE);
		}
		r = falcon_verify(sigct, sigct_len, FALCON_SIG_CT,
			pubkey, pubkey_len, "data1", 5, tmpvv, tmpvv_len);
		if (r != 0) {
			Serial.println(F("verify2(ct) failed"));	
			exit(EXIT_FAILURE);
		}
		if (logn >= 5) {
			r = falcon_verify(sigct, sigct_len, FALCON_SIG_CT,
				...
				tmpvv, tmpvv_len);
			if (r != FALCON_ERR_BADSIG) {
				Serial.println(F("wrong verify2(ct) err"));	
				exit(EXIT_FAILURE);
			}
		}
...
test_external_API(void)
{
	...
	Serial.print(F("Test external API: "));	
...
)
{
	...
	//già allocato fuori
	//msg = (uint8_t*)xmalloc(3300);

	sk = (uint8_t*) xmalloc(sk_len);
	pk = (uint8_t*)xmalloc(pk_len);
	sm = (uint8_t*)xmalloc(3300 + over_len);

	tmp = (uint8_t*)xmalloc((size_t)84 << logn);
	//esk = xmalloc((size_t)(8 * logn + 40) << logn);
	esk = (fpr*)xmalloc((size_t)(8 * logn + 40) << logn);
	...
	for (i = 0; i < 5; i ++) {
        ...
        Serial.println(CLOCK2-CLOCK1);
		endTime = micros();
        elapsedTime_test_nist_KAT = elapsedTime_test_nist_KAT + (endTime - startTime);
		...
		endTime = micros();
		CLOCK2 = ESP.getCycleCount();
        elapsedTime_encode = elapsedTime_encode + (endTime - startTime);
		Serial.print(" Encode private key: ");
        Serial.println(CLOCK2-CLOCK1);
		...
		endTime = micros();
		CLOCK2 = ESP.getCycleCount();
        elapsedTime_sig = elapsedTime_sig + (endTime - startTime);
		Serial.print(" Sign the message: ");
        Serial.println(CLOCK2-CLOCK1);
		...
		sm[1] = (uint8_t)u;
				endTime = micros();
        elapsedTime_vsig = elapsedTime_vsig + (endTime - startTime);
		...
		*/
        ...
        Serial.print(elapsedTime_test_nist_KAT);
        Serial.println("µs");
		Serial.print("Time enc: ");
        Serial.print(elapsedTime_encode);
        Serial.println("µs");
        Serial.print("Time sign: ");
        Serial.print(elapsedTime_sig);
        Serial.println("µs");
		Serial.print("Time verify sign: ");
        Serial.print(elapsedTime_vsig);
        Serial.println("µs");
        Serial.println("");
...
bool measureTime = false;  // Imposta a true per misurare il tempo, false per misurare i cicli di clock

#define WORD_NUMBER 1

 uint8_t parole_in_array[25][11] = {
...
void setup() {
    ...
    test_nist_KAT(9,parole_in_array,WORD_NUMBER);
