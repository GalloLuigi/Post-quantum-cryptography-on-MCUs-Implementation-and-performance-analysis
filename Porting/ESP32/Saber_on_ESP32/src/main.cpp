...
unsigned long elapsedTime_kp=0, elapsedTime_enc, elapsedTime_dec;
...
// Funzione per stampare l'array c
  ...
  for (int i = 0; i < WORD_NUMBER; i++)
    {
        ...
        startTime = micros();
        xTaskCreate(taskKeypair, "KeypairTask", 16000, NULL, 1, &taskHandleKeypair);
        endTime = micros();
        ...
        startTime = micros();
        xTaskCreate(taskEncrypt, "EncryptTask", 16000, NULL, 1, &taskHandleEncrypt);
        endTime = micros();
        ...
        startTime = micros();
        xTaskCreate(taskDecrypt, "DecryptTask", 17000, NULL, 1, &taskHandleDecrypt);
        endTime = micros();
