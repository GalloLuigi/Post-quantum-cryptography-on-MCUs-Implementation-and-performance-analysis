...
#include "hqc-192/api.h"
#include "hqc-192/parameters.h"
...
void setup()
{
    ...
    for (int i = 0; i < WORD_NUMBER; i++)
    {
        ...
        startTime = micros();
        xTaskCreate(taskKeypair, "KeypairTask", 22000, NULL, 1, &taskHandleKeypair);
        endTime = micros();
        ...
        startTime = micros();
        xTaskCreate(taskEncrypt, "EncryptTask", 35000, NULL, 1, &taskHandleEncrypt);
        endTime = micros();
        ...
        startTime = micros();
        xTaskCreate(taskDecrypt, "DecryptTask", 24000, NULL, 1, &taskHandleDecrypt);
        endTime = micros();
